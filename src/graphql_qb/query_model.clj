...
(defn update-binding [binding path-prefix path value]
  (let [[[key uri :as path-item] & ps] path]
    (if-let [[existing-uri spec] (get binding key)]
      (if (= uri existing-uri)
        (if (seq? ps)
          (let [updated (update-binding spec (conj path-prefix path-item) ps value)]
            (assoc binding key [uri updated]))
          ;;update existing match spec
          (assoc binding key [uri (update spec ::match update-binding-value value)]))
      ...
      (let [inner (if (seq? ps)
                    (update-binding {} (conj path-prefix path-item) ps value)
                    {::match value})]
        (assoc binding key [uri inner])))))

(defn add-binding [model path value]
  (if (= 0 (count path))
    model
    (update model :bindings (fn [b] (update-binding b [] path value)))))
...
(defn get-path-bgps [key-path parent [predicate binding-spec]]
  (let [match (::match binding-spec)
        nested (dissoc binding-spec ::match)
        ...
        child-bgps (mapcat (fn [[key binding-spec]]
                           ...
                           nested)]
    (conj child-bgps [parent predicate object])))
...
(defn format-query-triple-pattern [[s p o]]
  (str (query-format s) " " (query-format p) " " (query-format o) " ."))
...
(defn example []
  (-> empty-model
      ...
      (add-binding [[:dim1 sdmx:refArea] [:label rdfs:label]] ::var)
